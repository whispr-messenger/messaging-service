defmodule WhisprMessaging.Grpc.NotificationServiceClient do
  ...
  defp make_grpc_call(method, request, timeout \\ @default_timeout) do
    ...
    try do
      ...
      case :grpcbox_client.unary(
        ...
        "/notification.NotificationService/#{Macro.camelize(to_string(method))}",
        request,
        channel_opts
      ) do
        {:ok, response} ->
  ...
  defp get_service_config do
    %{
      host: Application.get_env(:whispr_messaging, :notification_service_host, "notification-service"),
      port: Application.get_env(:whispr_messaging, :notification_service_port, 50053),
  ...
  @doc """
  Initialise la connexion gRPC avec le notification-service
  """
  def start_connection do
    ...
    case :grpcbox_channel.start_link(channel_name, channel_opts) do
      {:ok, _pid} ->
        Logger.info("gRPC connection to notification-service established", %{
          host: config.host,
          port: config.port
        })
        :ok
      {:error, reason} ->
        Logger.error("Failed to establish gRPC connection to notification-service", %{
          reason: reason,
          host: config.host,
          port: config.port
        })
