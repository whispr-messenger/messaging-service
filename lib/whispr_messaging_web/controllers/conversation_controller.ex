defmodule WhisprMessagingWeb.ConversationController do
  ...
  defp render_conversation(conversation) do
    ...
    |> Map.put(:unread_count, Map.get(conversation, :unread_count, 0))
    |> Map.put(:last_message, render_last_message(Map.get(conversation, :last_message)))
  ...
  defp maybe_add_member_ids(rendered, conversation) do
    if conversation.type == "direct" do
      ...
      Map.put(rendered, :member_user_ids, member_ids)
    else
      rendered
    end
  end
