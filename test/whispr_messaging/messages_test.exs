defmodule WhisprMessaging.MessagesTest do
  ...
  describe "messages" do
    ...
    test "edit_message/4 updates message content", %{
      ...
      assert {:ok, updated_message} = Messages.edit_message(message.id, user1_id, new_content, new_metadata)
  ...
  describe "delivery statuses" do
    ...
    test "create_delivery_statuses_for_conversation/3 creates statuses for all members except sender", %{
      ...
      assert {:ok, count} = Messages.create_delivery_statuses_for_conversation(
        message.id,
        conversation.id,
        user1_id
      )
    ...
    test "mark_message_read/3 updates read status and sets delivered if not set", %{
      ...
      assert delivery_status.read_at != nil
      assert delivery_status.delivered_at != nil  # Should be set automatically
    end
    ...
    test "mark_conversation_read/3 marks all messages as read", %{
      ...
      assert count >= 3  # Should mark at least the 3 new messages
    end
  end
  ...
  describe "message helpers" do
    test "create_text_message/5 creates a text message" do
      ...
      assert %Ecto.Changeset{valid?: true} = Messages.create_text_message(
        conversation_id,
        sender_id,
        "encrypted_content",
        12345,
    ...
    test "create_system_message/3 creates a system message" do
      ...
      assert %Ecto.Changeset{valid?: true} = Messages.create_system_message(
        conversation_id,
        "User joined",
        %{"action" => "join"}
      )
